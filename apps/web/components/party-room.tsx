...
export function PartyRoom({ party, userId }: { party: Party; userId: string }) {
  ...
  const applyState = useCallback(({ isPlaying, timestamp, serverTime, contentType: incomingType, contentUrl: incomingUrl }: { isPlaying: boolean; timestamp: number; serverTime: number; contentType?: string; contentUrl?: string }) => {
    ...
    if (incomingType && incomingUrl) { setContentType(incomingType); setContentUrl(incomingUrl); }
  ...
  return <main className="min-h-screen overflow-x-hidden px-4 py-4 sm:px-7 sm:py-6">
    ...
    <section className="mx-auto mt-7 max-w-[1180px]">
      ...
      <div className="glow-orbit rounded-[22px] bg-[#0a1020] p-2.5"><div className="relative overflow-hidden rounded-[18px]">{contentType === "youtube" ? <YouTubePlayer videoId={videoId(contentUrl)} enabled={isHost} onReady={readyPlayer => { player.current = readyPlayer; }} onControl={control} /> : <video ref={uploadedPlayer} src={contentUrl} controls={isHost} className="aspect-video w-full bg-black" onPlay={() => control("play", uploadedPlayer.current?.currentTime || 0)} onPause={() => control("pause", uploadedPlayer.current?.currentTime || 0)} onSeeked={() => control("seek", uploadedPlayer.current?.currentTime || 0)} />}<div className="pointer-events-none absolute inset-x-0 bottom-0 flex items-end justify-between bg-gradient-to-t from-[#07101f]/90 via-transparent to-transparent px-4 pb-4 pt-16 sm:px-6"><span className="pointer-events-auto rounded-full border border-white/10 bg-[#10172b]/85 px-3 py-2 text-sm"><i className={`ml-2 inline-block h-2 w-2 rounded-full ${playing ? "now-pulse bg-[#ff7b8d]" : "bg-[#aab9d7]"}`} />{playing ? `${hostName} يشغّل الآن` : isHost ? "الفيديو جاهز — شغّله لما تكونوا مستعدين" : `${hostName} لم يبدأ التشغيل بعد`}</span><span className="mono rounded-full bg-[#10172b]/85 px-3 py-2 text-xs text-[#90e4ff]">{playing ? "LIVE" : "PAUSED"}</span></div></div></div>
