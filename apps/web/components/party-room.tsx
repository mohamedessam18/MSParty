...
export function PartyRoom({ party, userId }: { party: Party; userId: string }) {
  ...
  const [request, setRequest] = useState("سارة طلبت تتحكم لمدة قصيرة");
  const [copied, setCopied] = useState(false);
  const hostName = members.find(member => member.role === "host")?.user.name || "الهوست";

  const applyState = useCallback(({ isPlaying, timestamp, serverTime }: { isPlaying: boolean; timestamp: number; serverTime: number }) => {
  ...
  function togglePlayback() { const timestamp = player.current?.currentTime() || 0; if (playing) { player.current?.pause(); control("pause", timestamp); } else { player.current?.play(); control("play", timestamp); } }
  ...
  return <main className="min-h-screen overflow-x-hidden px-4 py-4 sm:px-7 sm:py-6">
    ...
    <section className="mx-auto mt-7 max-w-[1180px]">
      ...
      <div className="glow-orbit rounded-[22px] bg-[#0a1020] p-2.5"><div className="relative overflow-hidden rounded-[18px]"><YouTubePlayer videoId={videoId(party.contentUrl || "")} enabled={isHost} onReady={readyPlayer => { player.current = readyPlayer; }} onControl={control} /><div className="pointer-events-none absolute inset-x-0 bottom-0 flex items-end justify-between bg-gradient-to-t from-[#07101f]/90 via-transparent to-transparent px-4 pb-4 pt-16 sm:px-6"><span className="pointer-events-auto rounded-full border border-white/10 bg-[#10172b]/85 px-3 py-2 text-sm"><i className={`ml-2 inline-block h-2 w-2 rounded-full ${playing ? "now-pulse bg-[#ff7b8d]" : "bg-[#aab9d7]"}`} />{playing ? `${hostName} يشغّل الآن` : isHost ? "الفيديو جاهز — شغّله لما تكونوا مستعدين" : `${hostName} لم يبدأ التشغيل بعد`}</span><span className="mono rounded-full bg-[#10172b]/85 px-3 py-2 text-xs text-[#90e4ff]">{playing ? "LIVE" : "PAUSED"}</span></div></div></div>
